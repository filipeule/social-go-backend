...
func (s *UserStore) GetByID(ctx context.Context, userID int64) (*User, error) {
	query := `
		select
			users.id, username, email, password, created_at, roles.*
		from
			users
		JOIN roles on (users.role_id = roles.id)
		where
			users.id = $1 and is_active = true
	`
...
func (s *UserStore) CreateAndInvite(
	ctx context.Context, user *User, token string, invitationExp time.Duration,
) error {
	// transaction wrapper
...
func (s *UserStore) Activate(ctx context.Context, token string) error {
	return withTx(s.db, ctx, func(tx *sql.Tx) error {
		...
		// clear up the invitation from table
...
func (s *UserStore) getUserFromInvitation(
	ctx context.Context, tx *sql.Tx, token string,
) (*User, error) {
	query := `
...
func (s *UserStore) createUserInvitation(
	ctx context.Context, tx *sql.Tx, token string, invitationExp time.Duration, userID int64,
) error {
